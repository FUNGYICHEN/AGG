...
// test('URL/遊戲進入', async ({ browser, request }) => {
//   test.setTimeout(0);
...
//   // 為每個 base agent 加上前綴 "10" 與 "11"（例如：101 變成 10101 與 11101）
...
//       try {
//         // 取得遊戲 URL（若 API 返回 HTTP 4XX～5XX，generateGameUrl 會拋出錯誤）
//         const game_url = await generateGameUrl(request, agent, game_id);
//         console.log(`Agent: ${agent}, GameID: ${game_id} 取得 URL: ${game_url}`);

//         // 檢查 URL 是否以 expected_Wcasino 為前綴
//         if (!game_url.startsWith(expected_Wcasino)) {
//           console.error(`Agent: ${agent}, GameID: ${game_id} URL 前綴不符 -> ${game_url}`);
//           errorMessages.push(`Agent: ${agent}, GameID: ${game_id} URL 前綴不符 -> ${game_url}`);
//           continue;
//         }
//         console.log(`Agent: ${agent}, GameID: ${game_id} URL 正確: ${game_url}`);

//         // 開啟頁面，直接使用原始 game_url
//         const context = await browser.newContext();
//         const page = await context.newPage();
//         await page.goto(game_url, { waitUntil: 'load' });
//         console.log(`Agent: ${agent}, GameID: ${game_id} 正常進入遊戲`);
//         await context.close();
//       } catch (e) {
//         if (e.message.includes("HTTP錯誤")) {
//           console.log(`Agent: ${agent}, GameID: ${game_id} HTTP狀態碼錯誤: ${e.message}`);
//           errorMessages.push(`Agent: ${agent}, GameID: ${game_id} HTTP狀態碼錯誤: ${e.message}`);
//         } else {
...
//   } else {
//     console.log("所有 agent 測試成功，正常進入遊戲");
//   }
// });
